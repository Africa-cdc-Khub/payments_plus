...
use App\Services\ExchangeOAuth;
use Illuminate\Support\Facades\Log;
use Exception;

/**
 * Enhanced Exchange Email Service for Laravel
 * 
...
class ExchangeEmailService
{
    ...
    public function __construct()
    {
        ...
        $this->fromEmail = config('exchange-email.from_email');
        $this->fromName = config('exchange-email.from_name');
        $this->debugMode = config('exchange-email.debug', false);
    ...
    public function sendEmail($to, $subject, $body, $isHtml = true, $fromEmail = null, $fromName = null, $cc = [], $bcc = [], $attachments = [])
    {
        ...
        if (config('exchange-email.auth_method') === 'client_credentials' && !$this->oauth->hasValidToken()) {
    ...
    public function testConnection()
    {
        ...
        if (config('exchange-email.auth_method') === 'authorization_code') {
            $result['oauth_url'] = $this->getOAuthUrl();
        }

        if (!$result['has_tokens']) {
            if (config('exchange-email.auth_method') === 'client_credentials') {
    ...
    public function sendTestEmail($toEmail)
    {
        ...
        try {
            ...
            // Send test email
            $subject = "Enhanced Exchange Email Service Test - " . now()->format('Y-m-d H:i:s');
            ...
            if ($testResults['success']) {
                ...
                $testResults['details']['timestamp'] = now()->format('Y-m-d H:i:s');
            }
    ...
    {
        // If template is HTML content, use simple placeholder replacement
    ...
    {
        if ($this->debugMode) {
            Log::info('[ExchangeEmailService] ' . $message);
        }
    }
    ...
    {
        Log::error('[ExchangeEmailService] ' . $message);
    }
    ...
    {
        $authMethod = config('exchange-email.auth_method', 'client_credentials');
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            ...
            <div style="padding: 20px;">
                ...
                <ul>
                    ...
                    <li><strong>Authentication:</strong> ' . ucfirst(str_replace('_', ' ', $authMethod)) . '</li>
                    <li><strong>Security:</strong> Bearer Token Authentication</li>
                    <li><strong>Sent At:</strong> ' . now()->format('Y-m-d H:i:s T') . '</li>
                    <li><strong>Recipient:</strong> ' . htmlspecialchars($toEmail) . '</li>
                    <li><strong>From:</strong> ' . htmlspecialchars($fromEmail) . '</li>
            ...
            <div style="background: #f8f9fa; padding: 15px; text-align: center; font-size: 12px; color: #6c757d;">
                ...
                <p>Generated on ' . now()->format('Y-m-d H:i:s') . ' | Microsoft Graph API</p>
            </div>
        </body>
        </html>';
    }
     ...
     */
    protected function getInvitationTemplate()
    {
        return view()->exists('emails.invitation') 
            ? view('emails.invitation')->render()
            : '<html><body><h1>Invitation</h1><p>You have been invited.</p></body></html>';
    }
}
