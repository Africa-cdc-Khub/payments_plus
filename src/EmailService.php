...
class EmailService
{
    ...
    private $logoUrl;
    private $exchangeEmailService;

    public function __construct()
    {
        $this->templatePath = EMAIL_TEMPLATE_PATH;
        $this->logoUrl = EMAIL_LOGO_URL;
        $this->initializeExchangeService();
    }


    private function initializeExchangeService()
    {
        try {
            // Check if Exchange service is properly configured
            if (!file_exists(__DIR__ . '/../exchange-email.php')) {
                error_log("Exchange email configuration not found");
                $this->exchangeEmailService = null;
                return;
            }

            // Load Exchange Email Service classes
            require_once __DIR__ . '/ExchangeEmailService.php';
            require_once __DIR__ . '/ExchangeOAuth.php';
            
            // Load configuration from exchange-email.php
            $config = include __DIR__ . '/../exchange-email.php';
            
            // Check if required configuration is present
            if (empty($config['tenant_id']) || empty($config['client_id']) || empty($config['client_secret'])) {
                error_log("Exchange email configuration incomplete");
                $this->exchangeEmailService = null;
                return;
            }
            
            // Set environment variables if not already set
            if (!getenv('EXCHANGE_TENANT_ID') && isset($config['tenant_id'])) {
                putenv('EXCHANGE_TENANT_ID=' . $config['tenant_id']);
            }
            if (!getenv('EXCHANGE_CLIENT_ID') && isset($config['client_id'])) {
                putenv('EXCHANGE_CLIENT_ID=' . $config['client_id']);
            }
            if (!getenv('EXCHANGE_CLIENT_SECRET') && isset($config['client_secret'])) {
                putenv('EXCHANGE_CLIENT_SECRET=' . $config['client_secret']);
            }
            if (!getenv('EXCHANGE_REDIRECT_URI') && isset($config['redirect_uri'])) {
                putenv('EXCHANGE_REDIRECT_URI=' . $config['redirect_uri']);
            }
            if (!getenv('EXCHANGE_SCOPE') && isset($config['scope'])) {
                putenv('EXCHANGE_SCOPE=' . $config['scope']);
            }
            if (!getenv('EXCHANGE_AUTH_METHOD') && isset($config['auth_method'])) {
                putenv('EXCHANGE_AUTH_METHOD=' . $config['auth_method']);
            }
            if (!getenv('MAIL_FROM_ADDRESS') && isset($config['from_email'])) {
                putenv('MAIL_FROM_ADDRESS=' . $config['from_email']);
            }
            if (!getenv('MAIL_FROM_NAME') && isset($config['from_name'])) {
                putenv('MAIL_FROM_NAME=' . $config['from_name']);
            }

            $this->exchangeEmailService = new ExchangeEmailService($config);
            error_log("Exchange Email Service initialized successfully");
        } catch (Exception $e) {
            error_log("Exchange Email Service initialization failed: " . $e->getMessage());
            $this->exchangeEmailService = null;
        }
    }
    ...
    /**
     * Send email using Exchange Email Service or fallback
     */
    public function sendEmail($to, $subject, $htmlContent, $isHtml = true)
    {
        // Use Exchange Email Service (OAuth with automatic token refresh)
        if ($this->exchangeEmailService && $this->exchangeEmailService->isConfigured()) {
            try {
                $result = $this->exchangeEmailService->sendEmail($to, $subject, $htmlContent, $isHtml);
                if ($result) {
                    if (APP_DEBUG) {
                    ...
                    error_log("Exchange email failed for: {$to}");
                    return $this->sendFallbackEmail($to, $subject, $htmlContent, $isHtml);
                }
            } catch (Exception $e) {
                error_log("Exchange email error for {$to}: " . $e->getMessage());
                return $this->sendFallbackEmail($to, $subject, $htmlContent, $isHtml);
            }
        } else {
            error_log("Exchange Email Service not configured, using fallback");
            return $this->sendFallbackEmail($to, $subject, $htmlContent, $isHtml);
        }
    }

    /**
     * Send fallback email using PHP mail()
     */
    private function sendFallbackEmail($to, $subject, $htmlContent, $isHtml = true)
    {
        ...
        if ($result) {
            ...
            error_log("Fallback email failed for: {$to}");
        }
