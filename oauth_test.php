<?php
/**
 * OAuth Test and Email Sending
 * Handles OAuth flow and sends test email to andrewa@africacdc.org
 */
...
echo "<h2>CPHIA 2025 - OAuth Email Test</h2>";
...
try {
    ...
    if (isset($_GET['code']) && isset($_GET['state'])) {
        ...
        if ($success) {
            ...
            // Send test email
            $testSubject = 'CPHIA 2025 - OAuth Test Email';
            $testBody = '
            <html>
            ...
            $result = $oauth->sendEmail($testEmail, $testSubject, $testBody, true);
            ...
            if ($result) {
                ...
                echo "<p>Failed to send test email. Please check the error logs.</p>";
                echo "</div>";
            }
        ...
        if ($oauth->isConfigured()) {
            ...
            echo "<p><a href='" . $oauth->getAuthorizationUrl() . "' class='btn btn-primary' style='background: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>Authenticate with Microsoft</a></p>";
            echo "</div>";
        }
    }
